...
@Transaction 어노테이션은 우리가 아는 DB의 트랜젝션 기능을 하게 해준다

또한 수정기능을 했을때 객체를 수정하면 Save를 따로 해줘야 하지만 @Transaction 어노테이션을 적는다면 

굳이 save를 하지않아도 DB에서 감지하여 자동으로 변경해준다

근데 이 트랜젝션 어노테이션을 사용하는 메소드가 private로 선언되면 사용이 불가능하다 

Why?

Spring의 `@Transactional`은 **프록시 기반 AOP**(Aspect-Oriented Programming)로 동작합니다. 이 프록시는 **클래스 외부에서 메서드를 호출할 때만** 트랜잭션을 감지하고 적용할 수 있습니다.

- 즉, Spring은 해당 메서드를 감싼 **프록시 객체를 통해 호출될 때만** `@Transactional`을 적용합니다.
- 그런데 `private` 메서드는 외부에서 호출할 수 없고, 내부에서만 호출되기 때문에 **프록시를 타지 않아서 트랜잭션이 적용되지 않아요.**

AOP : "공통된 관심사(Cross-Cutting Concern)를 핵심 로직과 분리" 하여
코드를 깔끔하고 유지보수 가능하게 만드는 프로그래밍 방식입니다.

AOP의 활용 : 로그 찍기, 트랜잭션 처리, 보안 검사, 성능 측정 등은
→ 대부분 비즈니스 로직과 상관없는 공통 로직입니다.

AOP를 활용해 메소드 전,후, 작업을 제어할 수 있음 
